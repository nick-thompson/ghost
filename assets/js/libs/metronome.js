/*
 * Metronome.js v0.1.0
 * Copyright 2012 Nick Thompson; MIT License
 * Built upon the Oliver Caldwell's EventEmitter project (v4.0.2):
...
(function (exports, undefined) {
  ...
  // Metronome worker internal functions.
  // The tick interval is handled in a web worker to maintain consistency
  // even when the user switches tabs, or when the main thread gets overloaded.
  var 
      ...
      // To be inserted as a Blob 
      inline = "" +
        "self.addEventListener('message', " + msgHandler.toString() + ");";

  // Metronome object constructor.
  // @arg {int} bpm : Beats per minute
  // @arg {int} res : Resolution, metronome ticks per beat
  var Metronome = function (bpm, res) {
    this.bpm = bpm;
    this.res = res;
    this.ticks = 0;
  ...
  Metronome.prototype = new EventEmitter();
  ...
  Metronome.prototype.setResolution = function (res) {
    this.res = res;
  };

  // Set the tick value to start from. Unless manually set, the metronome
  // begins ticking from the value at which it left off, or zero.
  Metronome.prototype.startFrom = function (t) {
    this.ticks = t - 1;
  };
